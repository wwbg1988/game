...
<script type="text/javascript">
	var resourceTree;
	$(function() {
		var roleOwner =$.stringToList('${actionUserOwner}');
		if(roleOwner=='actionUserOwnerOk'){
			$('#actionUserOwnerId').attr('checked',true);
		}
		resourceTree = $('#resourceTree').tree({
			url : '${pageContext.request.contextPath}/actionsController/userTree',
			parentField : 'pid',
			//lines : true,
			checkbox : true,
		...
		$('#form').form({
			url : '${pageContext.request.contextPath}/actionsController/grantUsers',
			onSubmit : function() {
			...
			success : function(result) {
				...
				if (result.success) {
					//parent.$.modalDialog.openner_treeGrid.treegrid('reload');//之所以能在这里调用到parent.$.modalDialog.openner_treeGrid这	个对象，是因为role.jsp页面预定义好了
	...
	$("#searchNameBtn").click(function(){
		...
		resourceTree = $('#resourceTree').tree({
			url : '${pageContext.request.contextPath}/actionsController/userTree?searchName='+searchName,
			parentField : 'pid',
			//lines : true,
			checkbox : true,
			onClick : function(node) {
...
<div id="roleGrantLayout" class="easyui-layout" data-options="fit:true,border:false">
	<div data-options="region:'west'" title="用户列表" style="width: 300px; padding: 1px;">
		<div class="well well-small">
			<form id="form" method="post">
				...
				<input type="checkbox" id="actionUserOwnerId" name="ActionUserOwn" value="UserOwnOk">是否仅供提交表单组可见
			</form>
		</div>
	</div>
