...
<script type="text/javascript">
	$(function() {
		...
		var roleOwner =$.stringToList('${actionRoleOwner}');
		if(roleOwner=='actionRoleOwnerOk'){
			$('#actionRoleOwnerId').attr('checked',true);
		}
		$('#roleIds').combotree({
		...
		$('#form').form({
			url : '${pageContext.request.contextPath}/actionsController/grant',
			onSubmit : function() {
			...
			<table class="table table-hover table-condensed">
				<tr>
					...
					<td><select id="roleIds" name="roleIds" style="width: 140px; height: 29px;"></select><img src="${pageContext.request.contextPath}/style/images/extjs_icons/cut_red.png" onclick="$('#roleIds').combotree('clear');" /></td>
						<!-- <input type="checkbox" id="actionRoleOwnerId" name="ActionRoleOwn" value="roleOwnOk">是否仅供提交表单组可见 -->
				</tr>
			</table>
		</form>
	</div>
