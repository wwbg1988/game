...
<script type="text/javascript" charset="utf-8">
	...
	$(function() {
		...
		/* 		userLoginCombobox = $('#userLoginCombobox')
		 .combobox(
		 {
		 ...
		 userLoginCombogrid = $('#userLoginCombogrid')
		 .combogrid(
		 {
	...
	function loginFun() {
		if (layout_west_tree) {//当west功能菜单树加载成功后再执行登录
			...
			if (form.form('validate')) {
				...
				//如果数据库混乱 用这个controller方法去填充数据;'${pageContext.request.contextPath}/initController/init'
				$
						.post(
								'${pageContext.request.contextPath}/userController/login',
								form.serialize(),
								function(result) {
									if (result.success) {
								...
								/* 	if(!layout_west_virus_url){
									...
									if(!layout_west_advertisement_url){
										 layout_west_advertisement_url = ' ${pageContext.request.contextPath}/advertisementController/manager';
									} */
		...
		<div title="用户输入模式" style="overflow: hidden; padding: 10px;">
			<form method="post">
				<table class="table table-hover table-condensed">
					<tr>
						<th>登录名</th>
						<td><input name="userAccount" type="text" placeholder="请输入登录名"
				...
				<table class="table table-hover table-condensed">
					<tr>
						...
						<td><input id="userLoginCombobox" name="name" type="text"
							...
							data-options="required:true" value="超管" style="height: 29px;">
						</td>
					</tr>
					<tr>
						<th>密码</th>
...
<div id="defaultUserInfoDialog" title="系统测试账号"
	...
	<div class="well well-small" style="margin: 3px;">
		<div>
			<span class="badge">1</span>超管：超管/123456
		</div>
		<div>
