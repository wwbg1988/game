...
<script type="text/javascript">
	...
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${pageContext.request.contextPath}/cateringRoleController/dataGrid',
			fit : true,
			fitColumns : true,
			border : false,
			pagination : true,
			...
			},
			{
				...
				formatter : function(value, row, index) {
					...
					str += '&nbsp;';
							str += $.formatString('<img onclick="adduserFuns(\'{0}\');" src="{1}" title="绑定菜单"/>', row.id, '${pageContext.request.contextPath}/style/images/extjs_icons/key.png');
		...
		parent.$.messager.confirm('询问', '您是否要删除当前角色？', function(b) {
			if (b) {
				...
				if (currentUserId != id) {
					...
					$.post('${pageContext.request.contextPath}/cateringRoleController/deleteRole', {
						id : id
					}, function(result) {
						if (result.success) {
	...
	function editFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 700,
			height : 300,
			href : '${pageContext.request.contextPath}/cateringRoleController/editRole?id=' + id,
			buttons : [ {
				text : '编辑',
				handler : function() {
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 700,
			height : 500,
			href : '${pageContext.request.contextPath}/cateringRoleController/addPage',
			buttons : [ {
				text : '添加',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
	...
	function adduserFun(id) {
		...
		parent.$.modalDialog({
			...
			width : 520,
			height : 550,
			href : '${pageContext.request.contextPath}/cateringRoleController/grantUserPage?ids=' + id,
			buttons : [ {
				text : '授权',
				handler : function() {
	...
	function adduserFuns(id) {
		...
		parent.$.modalDialog({
			...
			width : 680,
			height : 800,
			href : '${pageContext.request.contextPath}/cateringRoleController/grantMenuPage?id=' + id,
			buttons : [ {
				text : '绑定',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为授权成功之后，需要刷新这个dataGrid，所以先预定义好
