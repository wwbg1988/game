...
<script type="text/javascript">
	...
	$(function() {
		treeGrid = $('#treeGrid').treegrid({
			url : '${pageContext.request.contextPath}/workMenuController/treeGrid',
			idField : 'id',
			treeField : 'name',
			parentField : 'pid',
			fit : true,
	...
	function deleteFun(id) {
		...
		if (node) {
			parent.$.messager.confirm('询问', '您是否要删除当前菜单？', function(b) {
				if (b) {
					parent.$.messager.progress({
						title : '提示',
						text : '数据处理中，请稍后....'
					});
					$.post('${pageContext.request.contextPath}/workMenuController/deleteMenu', {
						id : node.id
					}, function(result) {
						if (result.success) {
	...
	function editFun(id) {
		...
		if (node) {
			parent.$.modalDialog({
				...
				width : 500,
				height : 300,
				href : '${pageContext.request.contextPath}/workMenuController/editPage?id=' + node.id,
				buttons : [ {
					text : '编辑',
					handler : function() {
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 500,
			height : 350,
			href : '${pageContext.request.contextPath}/workMenuController/addPage',
			buttons : [ {
				text : '添加',
				handler : function() {
					parent.$.modalDialog.openner_treeGrid = treeGrid; //因为添加成功之后，需要刷新这个treeGrid，所以先预定义好
